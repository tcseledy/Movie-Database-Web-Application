    ...
    if ($global_method == "") {
      ...
      if (method_exists($this->serviceClass, $method)) {
        ...
        if (count($pArray) == $pCount && !in_array(null, $pArray)) {
          echo json_encode(call_user_func_array(array($this->serviceClass, $method), $pArray));
        }
        else {
          echo json_encode(array(
            "errorMessage" => "Required parameter(s) for $method: $paramStr"
          ));
        }
      }
      else {
        echo json_encode(array(
          "errorMessage" => "The method $method does not exist."
        ));
      }
    }
  }
